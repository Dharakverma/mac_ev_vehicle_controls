/*****************************************************************************************************************************/
/* MAIN S-FUNCTION for RTICANMM MainBlock (RTICANMM_MAIN_CAN_Driver.c) */
/*                                                                                                                           */
/*   AUTHOR(S):                                                                                                              */
/*      U. Homann                                                                                                              */
/*      M. Eikermann                                                                                                              */
...
#include "RTICANMM_MAIN_CAN_Driver_Globals.h"                                                                                                              
...
#define CANMM_CONTROLLER_ID CANMM_CTL_SETUP_Driver                                                                                             
...
#include "RTICANMM_MAIN_CAN_Driver_DEFS.h"                                                                                                              
#include "RTICANMM_MAIN_CAN_Driver_MSG_DATA.h"                                                                                                              
...
{                                                                                                                      
  ...
  static Int32 StartIdx[8] = {0, 5, 5, 5, 5, 5, 5, 5};                                                         
  static Int32 StopIdx[8]  = {4, 4, 4, 4, 4, 4, 4, 4};                                                         
  static Int32 NumOfIds[8] = {5, 0, 0, 0, 0, 0, 0, 0};                                                         
  Int32 MsgId = Msg->Id;                                                                                                     
  ...
  for (i=0; i<8; i++) {                                                                                            
    if( NumOfIds[MsgIDFormat]>0) {                                                                                 
      ...
      while (!found && (Start <= Stop)) {                                                                          
        ...
        Pos = (Start + Stop)>>1;                                                                                   
        if ((ID_Table_RX(Pos) == MsgId) && (Msg->Format == CANMMCAN_Driver_MIDFRX[Pos])) {                                             
          found = 1;                                                                                               
          break; // while                                                                                          
        }                                                                                                          
        else {                                                                                                     
...
{                                                                                                                          
  ...
  if (NUM_RX_OUTPUT) {                                                                                                     
#include "RTICANMM_MAIN_CAN_Driver_RX_PORT.h"                                                                                                              
  }                                                                                                                        
...
#include "RTICANMM_MAIN_CAN_Driver_TX_PORT.h"                                                                                                              
...
{                                                                                                                          
  ...
  int i = 0;                                                                                                               
#if !defined(MATLAB_MEX_FILE) || defined(CANMM_HOST_DEBUG)                                                            
#include "RTICANMM_MAIN_CAN_Driver_ID_TABLE.h"  /* fills the variable ID_Table with identifiers in rising order */                                          
#include "RTICANMM_MAIN_CAN_Driver_SIGNAL_DEFAULTS.h"                                                                                                              
#include "RTICANMM_MAIN_CAN_Driver_MESSAGE_DEFAULTS.h"                                                                                                              
  ...
  {                                                                                                                        
      ...
      {                                                                                                                
          msg_error_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MALLOC_ERROR , RTICANMM_MAINSFCN_MALLOC_ERROR_TXT, "CAN_Driver");
      }                                                                                                                
  }                                                                                                                        
  for (i = 0; i < NUM_TX_MSG; i++) {                                                                                   
...
{                                                                                                                          
  ...
  void **PWork = ssGetPWork(S);                                                                                            
      ...
      msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MDL_START_INFO , RTICANMM_MAINSFCN_MDL_START_INFO_TXT, "CAN_Driver");  
#endif                                                                                                                     
  ...
  msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_MDL_START_CYCLETIME_INFO, RTICANMM_MAINSFCN_MDL_START_CYCLETIME_INFO_TXT, "CAN_Driver", TX_CYCLETIME[0]);
#endif                                                                                                                 
...
/* Convert timestamp from HW time to model(current) time */                                                                
      ...
      if(!MsgData.CANMsg.Loopback) {                                                                                       
        msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_RX_LOOPBACK_INFO,                                                
                        RTICANMM_MAINSFCN_RX_LOOPBACK_INFO_TXT, "CAN_Driver",                                                      
                        MsgData.CANMsg.Id,                                                                                 
        ...
        msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_RX_MSG_INFO, RTICANMM_MAINSFCN_RX_MSG_INFO_TXT, "CAN_Driver",            
                        MsgData.CANMsg.Id,                                                                                 
                        MsgData.CANMsg.TimeStamp,                                                                          
                        MsgData.CANMsg.Dlc,                                                                                
                        MsgData.CANMsg.Data[0]);                                                                           
      ...
      if (idIdx != -1) {                                                                                           
        ...
        CANMMCAN_Driver_RXt[idIdx] = MsgData.CANMsg.TimeStamp;                   
        if (MsgData.CANMsg.Loopback) {                          
          CANMMCAN_Driver_RXdt[idIdx] = CANMMCAN_Driver_RXt[idIdx] - TX_OLDTIME[idIdx];            
          TX_OLDTIME[idIdx] = CANMMCAN_Driver_RXt[idIdx];                        
          CANMMCAN_Driver_RXsta[idIdx] = 1;                                    
        }                                                       
        else {                                                  
          CANMMCAN_Driver_RXdt[idIdx] = CANMMCAN_Driver_RXt[idIdx] - RX_OLDTIME[idIdx];            
          RX_OLDTIME[idIdx] = CANMMCAN_Driver_RXt[idIdx];                        
          CANMMCAN_Driver_RXsta[idIdx] = 1;                                        
        }                                                       
...
#include "RTICANMM_MAIN_CAN_Driver_TX_INPUT.h"                                                                                                          
...
/* Set status of message (if message could basically be sent) -------------------------- */                                
     ...
     {                                                                                                                  
      ...
      j = 0;                                                                                                           
      i = 0;                                                                                                           
      if (CANMMCAN_Driver_MAXMSGPERSTEP < 1) {                                                                                                    
          if (MAX_TX_PERSTEP < 1) {                                                                                    
            CANMMCAN_Driver_MAXMSGPERSTEP = 1;                                                                                                    
          }                                                                                                            
          else {                                                                                                       
            CANMMCAN_Driver_MAXMSGPERSTEP = MAX_TX_PERSTEP;                                                                                       
          }                                                                                                            
      }                                                                                                                
                                                                                                                       
      do { /* while ((i < NUM_TX_MSG) && (j < (CANMMCAN_Driver_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID))) */           
        UInt32 ID_number = TX_QUEUE(i).id;                                                                                 
        if (ID_number != NOCANID) {                                                                                      
          UInt8 TxMsgSent = 0;                                                                                             
                                                                                                                           
#if RTICANMM_LOGLEVEL > 3                                                                                                  
          msg_info_printf(MSG_SM_CANNEDY, RTICANMM_MAINSFCN_TX_MSG_INFO, RTICANMM_MAINSFCN_TX_MSG_INFO_TXT, "CAN_Driver",  ID_number);
#endif                                                                                                                     
                                                                                                                           
          switch (CANMMCAN_Driver_RXMT[ID_Table_TX(i)]) {                                                                                    
        ...
        i++;                                                                                                               
      } while ((i < NUM_TX_MSG) && (j < DPMEM_SIZE) && (j < (CANMMCAN_Driver_MAXMSGPERSTEP-(Int32)tx_queue_level)) && (j < CANMM_CTL_TX_QUEUE_SIZE(CANMM_CONTROLLER_ID)));       
    }                                                                                                                  
...
#include "RTICANMM_MAIN_CAN_Driver_RX_OUTPUT.h"                                                                                                          
